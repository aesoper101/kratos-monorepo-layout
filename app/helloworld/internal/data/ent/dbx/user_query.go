...
import (
	...
	"github.com/aesoper101/kratos-monorepo-layout/app/helloworld/internal/data/ent/dbx/predicate"
...
func (uq *UserQuery) Exist(ctx context.Context) (bool, error) {
	...
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("dbx: check existence: %w", err)
	default:
		return true, nil
	}
}
...
func (uq *UserQuery) prepareQuery(ctx context.Context) error {
	for _, inter := range uq.inters {
		if inter == nil {
			return fmt.Errorf("dbx: uninitialized interceptor (forgotten import dbx/runtime?)")
		}
	...
	for _, f := range uq.ctx.Fields {
		if !user.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("dbx: invalid field %q for query", f)}
		}
	}
	if uq.path != nil {
