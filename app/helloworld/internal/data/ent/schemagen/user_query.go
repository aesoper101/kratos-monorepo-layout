...
func (uq *UserQuery) prepareQuery(ctx context.Context) error {
	for _, f := range uq.fields {
		if !user.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("schemagen: invalid field %q for query", f)}
		}
	}
	if uq.path != nil {
...
func (uq *UserQuery) sqlExist(ctx context.Context) (bool, error) {
	...
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("schemagen: check existence: %w", err)
	default:
		return true, nil
	}
}
